...
export default function EditModal({ show, onClose, user, organizations }) {
    ...
    const { data, setData, processing, errors, setError, clearErrors, reset } = useForm({
        ...
        organization_id: user ? String(user.organization_id || '') : '',
    ...
    useEffect(() => {
        if (show && user) {
            ...
            setData('organization_id', String(user.organization_id || ''));
            setData('identity_type', user.identity_type || '');
            setData('identity_number', user.identity_number || '');
    ...
    function handleSubmit(e) {
        ...
        router.put(route('admin.staff.update', user.id), payload, {
            preserveScroll: true,
            onSuccess: () => onClose(),
        });
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between rounded-t-xl z-10">
                    <h2 className="text-lg font-bold text-gray-900">Edit: {user.name}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6" encType="multipart/form-data">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-xs font-medium text-gray-600 mb-1">
                                    Document Front <span className="text-gray-400">(image or PDF)</span>
                                </label>
                                {user.identity_document_front_url && !frontPreview && (
                                    <div className="mb-2">
                                        <a href={user.identity_document_front_url} target="_blank" rel="noopener noreferrer"
                                ...
                                <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 text-center hover:border-indigo-400 transition-colors cursor-pointer"
                                    onClick={() => document.getElementById('edit-front-upload').click()}>
                                    {frontPreview ? (
                                        <div className="relative">
                                ...
                                <input id="edit-front-upload" type="file" accept="image/*,.pdf" onChange={handleFrontFile} className="hidden" />
                            ...
                            {needsBack && (
                                <div>
                                    ...
                                    {user.identity_document_back_url && !backPreview && (
                                        <div className="mb-2">
                                            <a href={user.identity_document_back_url} target="_blank" rel="noopener noreferrer"
                                                className="text-xs text-indigo-600 hover:text-indigo-800 underline">View current document</a>
                                        </div>
                                    )}
                                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 text-center hover:border-indigo-400 transition-colors cursor-pointer"
                                        onClick={() => document.getElementById('edit-back-upload').click()}>
                                        {backPreview ? (
                                            <div className="relative">
                                    ...
                                    <input id="edit-back-upload" type="file" accept="image/*,.pdf" onChange={handleBackFile} className="hidden" />
