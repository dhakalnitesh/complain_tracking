            ...
            <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                                ...
                                {issue.extension_deadline_at && (
                                    <p className="text-xs text-amber-600">Extended deadline</p>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-200/60 p-5 sm:p-7 mb-5">
                    ...
                    <form onSubmit={handleProgressSubmit} className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                                    ...
                                    {progressPreview.map((url, i) => (
                                        <div key={i} className="relative">
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    setProgressPhotos(prev => prev.filter((_, j) => j !== i));
                                                    setProgressPreview(prev => prev.filter((_, j) => j !== i));
                                                }}
                        ...
                        <button
                            type="submit"
                            disabled={progressSubmitting || !progressNotes.trim()}
                        ...
                        <form onSubmit={handleExtensionSubmit} className="mb-4 p-4 bg-amber-50 rounded-xl border border-amber-200">
                            ...
                            <div className="mb-3">
                                ...
                                <input
                                    ...
                                    placeholder="e.g. 7"
                                    className="w-32 rounded-lg border-amber-200 border px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500 outline-none"
                                />
                            </div>
                            <div className="flex gap-2">
                                <button type="submit"
                                    disabled={extensionSubmitting || !extensionReason.trim() || !extensionDays}
                            ...
                            {issue.extension_requests.map(er => (
                                <div key={er.id} className="border border-gray-100 rounded-xl p-4">
                                    ...
                                    <p className="text-xs text-gray-500">
                                        Requested until: {new Date(er.requested_deadline).toLocaleDateString('en-GB')}
                                    </p>
                                    {er.admin_note && (
                                        <div className="mt-2 text-xs text-gray-500 bg-gray-50 rounded p-2">
                                            <span className="font-medium">Admin:</span> {er.admin_note}
                                        </div>
                                    )}
                                </div>
                            ))}
